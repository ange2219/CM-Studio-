  ...
  return (
    <div className="card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Link href="/notifications" style={{ fontSize: '0.73rem', color: 'var(--accent)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '2px', textDecoration: 'none' }}>
        ...
        {MOCK_NOTIFS.map(notif => (
          <div key={notif.id} style={{ display: 'flex', gap: '10px' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '0.78rem', color: 'var(--t1)', lineHeight: 1.4 }}>
                <span style={{ fontWeight: 700 }}>{notif.user}</span> {notif.text}
              </div>
              <div style={{ fontSize: '0.68rem', color: 'var(--t3)', marginTop: '2px' }}>{notif.time}</div>
            </div>
          </div>
        ))}
      </div>

      <Link href="/notifications" style={{ textAlign: 'center', fontSize: '0.78rem', color: 'var(--accent)', fontWeight: 600, textDecoration: 'none' }}>
