...
function MonthView({ monthDate, posts }: { monthDate: Date; posts: Post[] }) {
  ...
  function postsForDay(day: Date): Post[] {
    return posts.filter(p => {
      ...
      return sameDay(new Date(p.scheduled_at), day)
    })
  }
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                      {dayPosts.slice(0, 3).map(post => {
                        ...
                        return (
                          <div key={post.id} title={post.content} style={{
    ...
    <div style={{ margin: '0', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
          ...
          <span style={{ fontSize: '.88rem', fontWeight: 600, color: 'var(--t1)' }}>{headerLabel}</span>
      ...
      {view === 'week' && (
        <div style={{ overflowX: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '52px repeat(7, 1fr)', minWidth: '680px' }}>
            ...
            {weekDays.map((day, i) => {
              ...
              return (
                <div key={i} style={{ height: 52, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid var(--b1)', borderLeft: '1px solid var(--b1)' }}>
                  <span style={{ fontSize: '.66rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.06em', color: isToday ? 'var(--accent)' : 'var(--t3)' }}>
                    {DAY_LABELS[day.getDay()]} {day.getDate()}
                  </span>
                  {dayPosts.length > 0 && (
                    <div style={{ display: 'flex', gap: '3px', marginTop: '4px' }}>
                      {dayPosts.slice(0, 3).map(p => (
                        <div key={p.id} style={{ width: 6, height: 6, borderRadius: '50%', background: PLATFORM_COLORS[p.platforms[0]] || '#7B5CF5' }} />
                      ))}
                    </div>
                  )}
                </div>
            ...
            {HOURS.map(hour => (
              <Fragment key={hour}>
                ...
                {weekDays.map((day, di) => {
                  ...
                  return (
                    <div key={`${hour}-${di}`} style={{ height: HOUR_H, borderBottom: '1px solid var(--b1)', borderLeft: '1px solid var(--b1)', background: isToday ? 'rgba(123,92,245,.025)' : 'transparent', padding: '3px 5px' }}>
                      {cellPosts.map(post => {
                        const platform = post.platforms[0] || 'instagram'
                        const color    = PLATFORM_COLORS[platform] || '#7B5CF5'
                        const short    = PLATFORM_SHORT[platform] || platform
                        const topic    = post.content.trim().split(/\s+/).slice(0, 2).join(' ')
                        return (
                          <div key={post.id} title={post.content}
                            style={{ display: 'flex', alignItems: 'center', gap: '.28rem', padding: '.22rem .4rem', background: color + '1c', borderLeft: `3px solid ${color}`, borderRadius: '4px', fontSize: '.71rem', fontWeight: 600, marginBottom: '2px', cursor: 'pointer', overflow: 'hidden', whiteSpace: 'nowrap', transition: '.1s' }}
                            onMouseEnter={e => { e.currentTarget.style.background = color + '30' }}
                            onMouseLeave={e => { e.currentTarget.style.background = color + '1c' }}
                          >
                            <PlatformIcon platform={platform} size={11} />
                            <span style={{ color, flexShrink: 0 }}>{short}</span>
                            <span style={{ color: 'var(--t2)', fontWeight: 400, overflow: 'hidden', textOverflow: 'ellipsis' }}>· {topic}</span>
                          </div>
                        )
                      })}
                    </div>
      ...
      {/* Month view */}
      {view === 'month' && <MonthView monthDate={monthDate} posts={posts} />}
    </div>
  )
}
