...
export default function SettingsPage() {
  ...
  async function verifyCurrentPassword() {
    ...
    setPwdStep('verified')
    toast('Identit├® v├®rifi├®e', 'success')
  }

  async function changePassword() {
    if (newPassword.length < 8) { toast('8 caract├¿res minimum', 'error'); return }
    if (newPassword !== confirmPassword) { toast('Les mots de passe ne correspondent pas', 'error'); return }
    setSavingPwd(true)
    const { error } = await supabase.auth.updateUser({ password: newPassword })
    setSavingPwd(false)
    if (error) { toast(error.message, 'error'); return }
    toast('Mot de passe modifi├® avec succ├¿s', 'success')
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100%', margin: '-20px' }}>
      {/* ÔöÇÔöÇ Sidebar ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <aside style={{
        ...
        <div style={{ padding: '0 1.25rem', marginBottom: '1.25rem' }}>
          <h2 style={{ fontSize: '.78rem', fontWeight: 600, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '.07em' }}>Param├¿tres</h2>
        </div>
      ...
      {/* ÔöÇÔöÇ Contenu ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <main style={{ flex: 1, padding: '2rem 2.5rem', maxWidth: '680px' }}>

        {/* ÔöÇÔöÇ Apparence ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {active === 'appearance' && (
          <div>
            <SectionHeader title="Apparence" desc="Personnalisez l'affichage de l'application." />
            <SettingRow label="Th├¿me" desc="Choisissez entre le mode sombre et clair.">
        ...
        {/* ÔöÇÔöÇ Langue & Notifications ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {active === 'language' && (
          <div>
            <SectionHeader title="Langue & Notifications" desc="Pr├®f├®rences linguistiques et alertes email." />
            <SettingRow label="Langue" desc="Langue de l'interface.">
              <div style={{ display: 'flex', gap: '.5rem' }}>
                {([['fr', '­ƒç½­ƒçÀ Fran├ºais'], ['en', '­ƒç¼­ƒçº English']] as const).map(([code, label]) => (
                  <button key={code} onClick={() => { setLang(code); toast('Bient├┤t disponible', 'success') }} style={{
            ...
            <SettingRow label="Notifications email" desc="R├®sum├®s hebdomadaires, rappels de publication.">
              <Toggle value={emailNotifs} onChange={setEmailNotifs} />
            </SettingRow>
          </div>
        )}

        {/* ÔöÇÔöÇ Mot de passe ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {active === 'password' && (
          <div>
            <SectionHeader
              title="Mot de passe"
              desc="Modifiez votre mot de passe de connexion."
              badge={pwdStep === 'verified' ? <span style={{ display: 'flex', alignItems: 'center', gap: '.3rem', fontSize: '.73rem', color: '#22C55E', background: 'rgba(34,197,94,.08)', border: '1px solid rgba(34,197,94,.2)', padding: '.2rem .55rem', borderRadius: '999px' }}><CheckCircle size={11} /> Identit├® v├®rifi├®e</span> : undefined}
            />

            {pwdStep === 'idle' ? (
              <>
                <SettingRow label="Mot de passe actuel" desc="Confirmez votre identit├® avant de changer de mot de passe.">
                ...
                <div style={{ borderTop: '1px solid var(--b1)', paddingTop: '1.25rem' }}>
                  <button onClick={verifyCurrentPassword} disabled={savingPwd || !currentPassword} className="btn-primary flex items-center gap-2" style={{ opacity: !currentPassword ? .45 : 1 }}>
                    <Lock size={14} /> {savingPwd ? 'V├®rification...' : 'V├®rifier mon identit├®'}
                  </button>
                </div>
              </>
            ) : (
              <>
                <SettingRow label="Nouveau mot de passe" desc="8 caract├¿res minimum.">
                ...
                <SettingRow label="Confirmer">
                  <div style={{ maxWidth: '280px' }}>
                    <input className="input" type="password" placeholder="R├®p├®tez le mot de passe" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} style={{ borderColor: confirmPassword && confirmPassword !== newPassword ? 'rgba(239,68,68,.4)' : undefined }} />
        ...
        {/* ÔöÇÔöÇ Abonnement ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {active === 'billing' && (
          <div>
            <SectionHeader title="Abonnement" desc="G├®rez votre plan et vos informations de paiement." />
            ...
            <div style={{ padding: '1rem', background: 'var(--bg)', border: '1px solid var(--b1)', borderRadius: '10px', marginBottom: '1.5rem' }}>
              ...
              <div style={{ fontSize: '.78rem', color: 'var(--t3)' }}>
                {userPlan === 'free' ? '5 g├®n├®rations/jour ┬À Instagram & Facebook uniquement'
                  : userPlan === 'premium' ? '20 g├®n├®rations/jour ┬À 5 plateformes ┬À Planification'
                  : 'G├®n├®rations illimit├®es ┬À Toutes plateformes ┬À Workspaces'}
              </div>
            </div>
            ...
            {userPlan === 'free' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem' }}>
                <PlanCard
                  name="Premium" price="29Ôé¼/mois"
                  features={['20 g├®n├®rations / jour', '5 plateformes sociales', 'Planification avanc├®e', 'Analytiques']}
                  color="#4646FF"
                  onUpgrade={() => handleUpgrade('premium')}
                />
                <PlanCard
                  name="Business" price="79Ôé¼/mois"
                  features={['G├®n├®rations illimit├®es', 'Toutes les plateformes', 'Workspaces & ├®quipe', 'Support prioritaire']}
                  color="#FBBF24"
              ...
              <button onClick={handlePortal} className="btn-outline flex items-center gap-2">
                <CreditCard size={14} /> G├®rer mon abonnement
              </button>
            )}
          </div>
        )}

        {/* ÔöÇÔöÇ Zone dangereuse ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {active === 'danger' && (
          <div>
            <SectionHeader title="Zone dangereuse" desc="Actions irr├®versibles sur votre compte." danger />
            ...
            <div style={{ border: '1px solid rgba(239,68,68,.2)', borderRadius: '10px', overflow: 'hidden' }}>
              <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid rgba(239,68,68,.12)' }}>
                ...
                <p style={{ fontSize: '.78rem', color: 'var(--t3)', lineHeight: 1.6 }}>
                  La suppression est <strong style={{ color: '#ef4444' }}>d├®finitive et irr├®versible</strong>. Tous vos posts, donn├®es et connexions seront effac├®s imm├®diatement.
                </p>
              </div>
...
// ÔöÇÔöÇ Composants locaux ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    <div style={{ padding: '1rem', border: `1px solid ${color}25`, borderRadius: '10px', background: `${color}05` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '.75rem' }}>
        ...
        <button onClick={onUpgrade} style={{
          ...
          <ExternalLink size={13} /> Passer ├á {name}
        </button>
      </div>
