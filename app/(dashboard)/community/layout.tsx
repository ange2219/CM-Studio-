  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col gap-2 sm:gap-3 overflow-hidden select-none p-2 sm:p-0">
      ...
      <div className={`shrink-0 flex items-center justify-between px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl border transition-colors ${
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.id}
                href={tab.href}
                className={`flex items-center gap-2 px-3.5 py-1.5 text-[13px] font-bold rounded-lg transition-all no-underline ${
                  isActive
                    ? darkMode
                      ? 'bg-[#0F172A] text-[#38BDF8] shadow-xs border border-slate-700/90'
                      : 'bg-blue-50 text-[#1677FF] shadow-xs border border-blue-200'
                    : darkMode
        ...
        <div className={`hidden sm:flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-semibold border ${
          darkMode 
            ? 'bg-[#0F172A] border-slate-700/80 text-slate-300' 
